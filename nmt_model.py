...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = nn.utils.rnn.pack_padded_sequence(input=self.model_embeddings.source(source_padded), lengths=source_lengths)
        enc_hiddens, (h_n, c_n) = self.encoder(X)
        enc_hiddens, enc_hiddens_unpack_len = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        h_n = h_n.permute(1, 0, 2)
        init_decoder_hidden = self.h_projection(h_n.contiguous().view(h_n.shape[0], -1))
        c_n = c_n.permute(1, 0, 2)
        init_decoder_cell = self.c_projection(c_n.contiguous().view(c_n.shape[0], -1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            Ybar_t = torch.cat((Y_t_sq, o_prev), 1)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t_expand = torch.unsqueeze(alpha_t, 2)
        a_t = torch.bmm(alpha_t_expand.permute(0, 2, 1), enc_hiddens)
